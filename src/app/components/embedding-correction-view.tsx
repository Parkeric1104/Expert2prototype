  ...
  const renderDocPage = (pageText: string) => {
    ...
    return (
      <pre className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap font-sans">
        {parts.map((part, i) =>
          part === keyword ? (
            <mark key={i} className="bg-indigo-100 text-indigo-900 rounded px-1">
              {part}
            </mark>
          ) : (
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="border-b border-border bg-card px-6 py-3 flex-shrink-0">
        <div className="flex items-center gap-6">
          ...
          <span className="flex items-center gap-1.5 text-sm text-muted-foreground">
            ...
            <span className="font-medium text-foreground">총 청크</span>
            <span className="ml-1 font-semibold text-foreground">{chunks.length}개</span>
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-1/2 flex flex-col border-r border-border bg-card overflow-hidden">
          <div className="px-5 py-3 border-b border-border flex items-center gap-2 flex-shrink-0 bg-muted/30">
            <BookOpen className="w-4 h-4 text-primary" />
            <span className="text-sm font-semibold text-foreground">원문 문서</span>
            <span className="ml-auto text-xs text-muted-foreground bg-background border border-border px-2 py-1 rounded">
          ...
          {/* 원문 페이지 네비 */}
          <div className="border-t border-border px-5 py-3 flex items-center justify-center gap-2 flex-shrink-0 bg-muted/30">
            <Button
              variant="ghost"
              size="sm"
            ...
            <span className="text-xs text-muted-foreground">원문 {docViewPage}페이지</span>
            <Button
              variant="ghost"
              size="sm"
              className="h-8 w-8 p-0"
        ...
        {/* 우측: 청크 리스트 */}
        <div className="w-1/2 flex flex-col overflow-hidden bg-muted/50">
          <div className="px-5 py-3 border-b border-border flex items-center gap-2 flex-shrink-0 bg-muted/30">
            <Database className="w-4 h-4 text-primary" />
            <span className="text-sm font-semibold text-foreground">청크 데이터</span>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
            {pageChunks.map((chunk, index) => {
              ...
              return (
                <div
                  key={chunk.id}
                  className={`bg-card border rounded-lg p-4 shadow-sm cursor-pointer transition-all ${
                ...
                >
                  ...
                  <div className="flex justify-between items-center mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-muted-foreground">{chunk.tokens}t</span>
                    </div>
                    ...
                    <div className="flex items-center gap-0.5" onClick={(e) => e.stopPropagation()}>
                      {!isEditing ? (
                        ...
                        <>
                          ...
                          <Button size="sm" className="h-7 px-2 text-xs" onClick={() => saveEditing(chunk.id)}>
                  ...
                  {isEditing ? (
                    <Textarea
                      ...
                      className="min-h-[72px] text-sm p-2.5 border-primary focus-visible:ring-primary bg-primary/5"
                      autoFocus
                      onClick={(e) => e.stopPropagation()}
                    />
                  ) : (
                    <div className={`text-sm leading-relaxed p-2.5 rounded-lg border ${
                      isSelected ? "bg-primary/5 border-primary/20 text-foreground" : "bg-muted border-border text-muted-foreground"
                    }`}>
                      {chunk.content}
                    </div>
                  )}
          ...
          {/* ── 하단 페이지네이션 ── */}
          <div className="border-t border-border px-5 py-3 flex items-center justify-center gap-2 flex-shrink-0 bg-card">
            <Button
              variant="ghost"
              size="sm"
              className="h-8 w-8 p-0"
