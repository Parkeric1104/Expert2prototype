...
- 전체 화면 배경은 그라데이션 사용: `linear-gradient(180deg, #FFFFFF 0%, #FFFFFF 35%, #EBF1FF 100%)` — 하단은 **Blue 100 `#EBF1FF`**(단색 흰 배경만 쓰지 말 것). App 루트에 적용돼 있으며, 새 전체화면 뷰는 배경을 투명히 두어 상속받거나 동일 그라데이션을 준다.
- **Primary = Blue 500 `#5784FF`** (디자인 시스템 Color.png 2026.04.29 반영, 기존 인디고 `#6366F1`에서 전환). `#3182F6`(토스 블루)는 **의견서/플로팅 버튼 전용**으로 별도 유지. 신규 Blue 150 = `#E5EEFF`.
